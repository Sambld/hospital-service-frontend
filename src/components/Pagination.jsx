        ...
        <Center mt={5}>
            <HStack>
                {pagination.current_page != 1 && pagination.current_page != 2 && (
                    <HStack>
                        <Button bg={useColorModeValue('gray.500', 'gray.200')} onClick={() => { action(1) }}> 1 </Button>
                        {pagination.current_page != 3 && <Text color={useColorModeValue('gray.900', 'white')}>...</Text>}
                    </HStack>
                )}
                {pagination.current_page != 1 && (
                    <Button bg={useColorModeValue('gray.500', 'gray.200')} onClick={() => { action(pagination.current_page - 1) }}> {(pagination.current_page - 1)} </Button>
                )}
                {/* <Input type='number' defaultValue={pagination.current_page} onKeyDown={handleKeyPress} w='100px' textAlign='center' /> */}
                {pagination.last_page == 2 ? (
                    <Button bg={useColorModeValue('gray.500', 'gray.200')} colorScheme='blue' isDisabled> {pagination.current_page} </Button>
                ) : (
                    <Select defaultValue={pagination.current_page} onChange={(e) => { action(e.target.value) }} textAlign='center' bg={useColorModeValue('gray.500', 'gray.200')} colorScheme='blue' w='100px'>
                ...
                {pagination.current_page != pagination.last_page && pagination.current_page != (pagination.last_page - 1) && (
                    <HStack>
                        ...
                        <Button bg={useColorModeValue('gray.500', 'gray.200')} onClick={() => { action(pagination.last_page) }}> {pagination.last_page} </Button>
                    </HStack>
                )}
            </HStack>
        </Center>
